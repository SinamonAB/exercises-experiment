    ...
    $(document).ready(() => { waitForElement(".lesson-content").then(() => {
        ...
        const exerciseList = [
            ...
            },
            {
                ...
                answer_options: [
                    { chinese: "a. 因为她有很多猫" },
                    { chinese: "b. 因为她的生日是明天"},
                    { chinese: "c. 因为她的男朋友来了", correct: true },
                    { chinese: "d. 因为她在瑞典住"}
                ]
            }
        ];
                    ...
                    <div class="exercise-progress">
                        <div>
                            <ol class="progress-icons">
                                ${exerciseList.map((exerciseJson, exerciseNum) => { return `<li class="progress-icon ahead" data-exercise-num="${exerciseNum}"></li>`; }).join(" ")}
                            </ol>
                        </div>
                        <div>
                    ...
                    task = `
                        <div class="exercise-task">
                            ...
                            <ol class="answer-options">
                                ${questionJson.answer_options.map((option, optionNum) => { return `<li><button type="button" class="btn choice-option-btn" data-option-num="${optionNum}">${option.chinese}</button></li>`; }).join("")}
                            </ol>
                        </div>
        ...
        const updateExerciseState = (exerciseJson, newState) => {
            ...
            const initial = () => {
                ...
                exerciseJson.el.find(".skip-btn").click(() => { completeExercise(exerciseJson, CompletionStatus.Skipped); });
            };
            switch (newState) {
                ...
                case ReadingExerciseState.Checking:
                    ...
                    exerciseJson.el.find(".check-btn").click(() => { completeExercise(exerciseJson, completionStatus); });
                    break;
            }
